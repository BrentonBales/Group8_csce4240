...
while True:
    ...
    if len(faces) > 0:
        for (x, y, w, h) in faces:
            ...
            coords = (x, y)
            closest_person = min(people_info, key=lambda l: (l.xy[0] - coords[0])**2 + (l.xy[1] - coords[1])**2, default=None)
            if closest_person is not None:
                closest_coords = closest_person.xy
                if abs(coords[0] - closest_coords[0]) <= face_distance_x and abs(coords[1] - closest_coords[1]) <= face_distance_y:
